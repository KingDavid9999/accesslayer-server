...
import { getCurrentFeeTier, updateFeeTierConfig, loadFeeTierConfig } from './fee-tier.service';
import { logger } from '../../utils/logger.utils';

const updateFeeTierBodySchema = z.object({
   tiers: z.array(
      z.object({
         volumeThreshold: z.number().nonnegative('volumeThreshold must be non-negative'),
         feeBps: z.number().int().min(1, 'feeBps must be at least 1').max(10000, 'feeBps must not exceed 10000'),
...
router.patch(
   ...
   async (req, res, next) => {
      ...
      try {
         ...
         sendSuccess(res, { tiers: updatedTiers }, 200, 'Fee tiers updated successfully');
      } catch (error) {
